...
pub fn get(db: &Db, id: &str) -> Result<Option<DecisionRow>> {
    db.with(|conn| {
        let row = conn
            .query_row(
                "SELECT id, project_id, decision, reasoning, topic_key, revision_count, status, importance, obsolete_reason, token_count, tokenizer_model, content_hash, created_at, updated_at FROM decisions WHERE id = ?",
                params![id],
                |r| {
                    Ok(DecisionRow {
                        id: r.get(0)?,
